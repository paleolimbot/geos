...
test_that("geo_tbl_polygon() c() and vec_c() works", {
  polygon <- geo_tbl_polygon(geo_xy(c(0, 1, 6), c(0, 2, 4)))
  ...
  expect_is(c(polygon, polygon), "geo_tbl_polygon")
  expect_length(c(polygon, polygon), 6)
  expect_is(vec_c(polygon, polygon), "geo_tbl_polygon")
  expect_length(vec_c(polygon, polygon), 6)
  expect_equal(nrow(vec_rbind(polygon_in_tbl, polygon_in_tbl)), 6)
  expect_is(vec_rbind(polygon_in_tbl, polygon_in_tbl)$polygon, "geo_tbl_polygon")
...
test_that("geo_tbl_polygon() casting works", {
  polygon <- geo_tbl_polygon(geo_xy(c(0, 1, 6), c(0, 2, 4)))
  tbl_polygon <- as_tibble(polygon)
  ...
  expect_identical(vec_cast(tbl_polygon, new_geo_tbl_polygon()), polygon)
  ...
  expect_identical(vec_cast(vec_data(tbl_polygon), new_geo_tbl_polygon()), polygon)
  expect_error(
    vec_cast(unname(vec_data(tbl_polygon)), new_geo_tbl_polygon()),
    "Can't convert an unnamed list"
  )

  expect_identical(as_geo_tbl_polygon(tbl_polygon), polygon)
  expect_identical(as_geo_tbl_polygon(df_polygon), polygon)
  expect_identical(as_geo_tbl_polygon(unclass(df_polygon)), polygon)
})


test_that("geo_tbl_multipolygon() c() and vec_c() works", {
  multipolygon <- geo_tbl_multipolygon(geo_xy(c(0, 1, 6), c(0, 2, 4)))
  ...
  expect_is(c(multipolygon, multipolygon), "geo_tbl_multipolygon")
  expect_length(c(multipolygon, multipolygon), 6)
  expect_is(vec_c(multipolygon, multipolygon), "geo_tbl_multipolygon")
...
test_that("geo_tbl_multipolygon() casting works", {
  multipolygon <- geo_tbl_multipolygon(geo_xy(c(0, 1, 6), c(0, 2, 4)))
  tbl_multipolygon <- as_tibble(multipolygon)
  ...
  expect_identical(
    vec_cast(tbl_multipolygon, new_geo_tbl_multipolygon()),
    multipolygon
  )
  ...
  expect_identical(vec_cast(vec_data(tbl_multipolygon), new_geo_tbl_multipolygon()), multipolygon)
  expect_error(
    vec_cast(unname(vec_data(tbl_multipolygon)), new_geo_tbl_multipolygon()),
    "Can't convert an unnamed list"
  )

  expect_identical(as_geo_tbl_multipolygon(tbl_multipolygon), multipolygon)
  expect_identical(as_geo_tbl_multipolygon(df_multipolygon), multipolygon)
  expect_identical(as_geo_tbl_multipolygon(unclass(df_multipolygon)), multipolygon)
})
...
test_that("polygon is can be coerced to multipolygon", {
  ...
  expect_is(c(multipolygon, polygon), "geo_tbl_multipolygon")
  expect_is(c(polygon, multipolygon), "geo_tbl_multipolygon")
  expect_is(vec_c(multipolygon, polygon), "geo_tbl_multipolygon")
  expect_is(vec_c(polygon, multipolygon), "geo_tbl_multipolygon")
})
