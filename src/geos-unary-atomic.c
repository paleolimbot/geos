...
SEXP geos_c_area(SEXP geom) { GEOS_UNARY_REAL(GEOSArea_r, 0); }

SEXP geos_c_length(SEXP geom) { GEOS_UNARY_REAL(GEOSLength_r, 0); }

SEXP geos_c_x(SEXP geom) { GEOS_UNARY_REAL(GEOSGeomGetX_r, 0); }

SEXP geos_c_y(SEXP geom) { GEOS_UNARY_REAL(GEOSGeomGetY_r, 0); }

SEXP geos_c_z(SEXP geom) { GEOS_UNARY_REAL(GEOSGeomGetZ_r, 0); }

SEXP geos_c_xmin(SEXP geom) { GEOS_UNARY_REAL(GEOSGeom_getXMin_r, 0); }

SEXP geos_c_ymin(SEXP geom) { GEOS_UNARY_REAL(GEOSGeom_getYMin_r, 0); }

SEXP geos_c_xmax(SEXP geom) { GEOS_UNARY_REAL(GEOSGeom_getXMax_r, 0); }

SEXP geos_c_ymax(SEXP geom) { GEOS_UNARY_REAL(GEOSGeom_getYMax_r, 0); }

SEXP geos_c_minimum_clearance(SEXP geom) { GEOS_UNARY_REAL(GEOSMinimumClearance_r, 2); }
...
SEXP geos_c_extent(SEXP geom) {
  ...
  for (R_xlen_t i = 0; i < size; i++) {
    ...
    int resultCode =
        GEOSGeom_getExtent_r(handle, geometry, xmin + i, ymin + i, xmax + i, ymax + i);
...
SEXP geos_c_hilbert_code(SEXP geom, SEXP geomExtent, SEXP level_sexp) {
  ...
  for (R_xlen_t i = 0; i < size; i++) {
    ...
    int resultCode = GEOSHilbertCode_r(handle, geometry, extent, level, &item_code);
...
#define GEOS_UNARY_RETURN(_func, _scalar_type, _vec_type, _vec_ptr, _na_value, \
                          _errorValue)                                         \
  R_xlen_t size = Rf_xlength(geom);                                            \
...
SEXP geos_c_coorinate_dimension(SEXP geom) {
  ...
  GEOS_UNARY_RETURN(GEOSGeom_getCoordinateDimension_r, int, INTSXP, INTEGER, NA_INTEGER,
                    0);
}
...
SEXP geos_c_is_valid_detail(SEXP geom, SEXP allowSelfTouchingRingFormingHole) {
  ...
  for (R_xlen_t i = 0; i < size; i++) {
    ...
    validResult =
        GEOSisValidDetail_r(handle, geometry, flags, &reasonResult, &geometryResult);
    ...
    if (validResult == 2) {
      ...
      pResultIsValid[i] = 1;
      SET_STRING_ELT(resultReason, i, NA_STRING);
      SET_VECTOR_ELT(resultLocation, i,
                     geos_common_geometry_xptr(GEOSGeom_createEmptyCollection_r(
