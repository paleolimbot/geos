...
#define GEOS_ERROR(msg, arg)                                           \
  char actualErrorMessage[GEOS_ACTUAL_ERROR_MESSAGE_BUFFER_SIZE];      \
  strcpy(actualErrorMessage, msg);                                     \
  memcpy(&actualErrorMessage[strlen(msg)], globalErrorMessage,         \
         strlen(globalErrorMessage));                                  \
...
#define ERROR_OLD_LIBGEOS_BUILD(capability, version)                                     \
  Rf_error(                                                                              \
      ...
      "was built against 'libgeos' %s.\nTo fix, run `install.packages(c(\"libgeos\", "   \
      "\"geos\"))`",                                                                     \
