...
List geometry_to_geo_tbl(GEOSContextHandle_t context, GEOSGeometry* geometry, int feature) {
  ...
  if (type == GEOSGeomTypes::GEOS_POINT) {
    geoTbl = point_to_geo_tbl(context, geometry, feature);

  } else if(type == GEOSGeomTypes::GEOS_LINESTRING) {
    geoTbl = linestring_to_geo_tbl(context, geometry, feature);
    ...
    geoTbl = multipoint_to_geo_tbl(context, geometry, feature);

  } else if(type == GEOSGeomTypes::GEOS_MULTILINESTRING) {
    geoTbl = multilinestring_to_geo_tbl(context, geometry, feature);
