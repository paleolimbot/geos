...
SEXP geos_c_centroid(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSGetCentroid_r); }

SEXP geos_c_boundary(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSBoundary_r); }

SEXP geos_c_minimum_width(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSMinimumWidth_r); }
...
SEXP geos_c_unary_union(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSUnaryUnion_r); }

SEXP geos_c_coverage_union(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSCoverageUnion_r); }

SEXP geos_c_point_on_surface(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSPointOnSurface_r); }

SEXP geos_c_node(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSNode_r); }

SEXP geos_c_make_valid(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSMakeValid_r); }

SEXP geos_c_unique_points(SEXP geom) {
  GEOS_UNARY_GEOMETRY(GEOSGeom_extractUniquePoints_r);
}

SEXP geos_c_reverse(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSReverse_r); }

SEXP geos_c_merge_lines(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSLineMerge_r); }

SEXP geos_c_build_area(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSBuildArea_r); }

SEXP geos_c_envelope(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSEnvelope_r); }

SEXP geos_c_convex_hull(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSConvexHull_r); }

SEXP geos_c_point_start(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSGeomGetStartPoint_r); }

SEXP geos_c_point_end(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSGeomGetEndPoint_r); }

SEXP geos_c_line_merge(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSLineMerge_r); }

SEXP geos_c_clone(SEXP geom) { GEOS_UNARY_GEOMETRY(GEOSGeom_clone_r); }
...
static int transform_callback(double* x, double* y, void* userdata) {
  ...
  int result =
      data->trans->trans(data->i, data->xyzm_in, data->xyzm_out, data->trans->trans_data);
...
SEXP geos_c_interpolate(SEXP geom, SEXP param) {
  GEOS_UNARY_GEOMETRY_PARAM(GEOSInterpolate_r(handle, geometry, paramPtr[i]), double,
                            REAL, ISNA(paramPtr[i]));
}

SEXP geos_c_interpolate_normalized(SEXP geom, SEXP param) {
  GEOS_UNARY_GEOMETRY_PARAM(GEOSInterpolateNormalized_r(handle, geometry, paramPtr[i]),
                            double, REAL, ISNA(paramPtr[i]));
}

SEXP geos_c_point_n(SEXP geom, SEXP param) {
  GEOS_UNARY_GEOMETRY_PARAM(GEOSGeomGetPointN_r(handle, geometry, paramPtr[i]), int,
                            INTEGER, paramPtr[i] == NA_INTEGER);
}

SEXP geos_c_simplify(SEXP geom, SEXP param) {
  GEOS_UNARY_GEOMETRY_PARAM(GEOSSimplify_r(handle, geometry, paramPtr[i]), double, REAL,
                            ISNA(paramPtr[i]));
}

SEXP geos_c_simplify_preserve_topology(SEXP geom, SEXP param) {
  GEOS_UNARY_GEOMETRY_PARAM(GEOSTopologyPreserveSimplify_r(handle, geometry, paramPtr[i]),
                            double, REAL, ISNA(paramPtr[i]));
}
...
SEXP geos_c_unary_union_prec(SEXP geom, SEXP param) {
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSUnaryUnionPrec_r(handle, geometry, paramPtr[i]), double,
                            REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_maximum_inscribed_circle(SEXP geom, SEXP param) {
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSMaximumInscribedCircle_r(handle, geometry, paramPtr[i]),
                            double, REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_densify(SEXP geom, SEXP param) {
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSDensify_r(handle, geometry, paramPtr[i]), double, REAL,
                            ISNA(paramPtr[i]));
#else
...
SEXP geos_c_remove_repeated_points(SEXP geom, SEXP param) {
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSRemoveRepeatedPoints_r(handle, geometry, paramPtr[i]),
                            double, REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_set_precision(SEXP geom, SEXP param, SEXP preserveTopology,
                          SEXP keepCollapsed) {
  int flags = 0;
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSGeom_setPrecision_r(handle, geometry, paramPtr[i], flags),
                            double, REAL, ISNA(paramPtr[i]));
}
...
SEXP geos_c_concave_hull(SEXP geom, SEXP param, SEXP allowHoles_sexp) {
  ...
  GEOS_UNARY_GEOMETRY_PARAM(GEOSConcaveHull_r(handle, geometry, paramPtr[i], allowHoles),
                            double, REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_concave_hull_of_polygons(SEXP geom, SEXP param, SEXP isTight_sexp,
                                     SEXP allowHoles_sexp) {
#if LIBGEOS_VERSION_COMPILE_INT >= LIBGEOS_VERSION_INT(3, 11, 0)
  if (libgeos_version_int() < LIBGEOS_VERSION_INT(3, 11, 0)) {
  ...
  int allowHoles = INTEGER(allowHoles_sexp)[0];
  GEOS_UNARY_GEOMETRY_PARAM(
      GEOSConcaveHullOfPolygons_r(handle, geometry, paramPtr[i], isTight, allowHoles),
      double, REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_polygon_hull_simplify(SEXP geom, SEXP param, SEXP isOuter_sexp,
  ...
  GEOS_UNARY_GEOMETRY_PARAM(
      GEOSPolygonHullSimplifyMode_r(handle, geometry, isOuter, mode, paramPtr[i]), double,
      REAL, ISNA(paramPtr[i]));
#else
...
SEXP geos_c_clip_by_rect(SEXP geom, SEXP xmin, SEXP ymin, SEXP xmax, SEXP ymax) {
  ...
  for (R_xlen_t i = 0; i < size; i++) {
    ...
    if (item == R_NilValue || ISNA(pXmin[i]) || ISNA(pYmin[i]) || ISNA(pXmax[i]) ||
        ISNA(pYmax[i])) {
      SET_VECTOR_ELT(result, i, R_NilValue);
      continue;
    }
    ...
    geometryResult =
        GEOSClipByRect_r(handle, geometry, pXmin[i], pYmin[i], pXmax[i], pYmax[i]);
...
SEXP geos_c_voronoi_diagram(SEXP geom, SEXP env, SEXP tolerace, SEXP edges) {
  ...
  for (R_xlen_t i = 0; i < size; i++) {
    ...
    geometryResult =
        GEOSVoronoiDiagram_r(handle, geometry, envGeometry, dTolerance, iEdges);
...
SEXP geos_c_offset_curve(SEXP geom, SEXP distance, SEXP params) {
  GEOS_BUFFER(
      GEOSOffsetCurve_r(handle, geometry, pDistance[i], quadSegs, joinStyle, mitreLimit));
}
