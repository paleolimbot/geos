...
#' @export
#'
new_geo_tbl_polygon <- function(x = list(xy = geo_xy(), feature = integer(0))) {
  vec_assert(x$xy, geo_xy())
  vec_assert(x$piece, integer())
  vec_assert(x$feature, integer())
  new_rcrd(x, class = "geo_tbl_polygon")
}

#' @rdname new_geo_tbl_polygon
#' @export
new_geo_tbl_multipolygon <- function(x = list(xy = geo_xy(), feature = integer(0))) {
  vec_assert(x$xy, geo_xy())
  vec_assert(x$feature, integer())
  vec_assert(x$piece, integer())
  vec_assert(x$part, integer())
  new_rcrd(x, class = "geo_tbl_multipolygon")
}
